...
func (c *Controller) HandleUserActionsFormPost(gctx *gin.Context) {
	...
	if action == "deactivate" {
		...
		if email == "" || name == "" || !strings.Contains(email, "@") || len(name) < 3 {
			...
			models.SendPasswordResetToken(c.GetDB(gctx), email, "Your gosass account is ready.", "newuser")
...
func (c *Controller) SetupIntialUser(gctx *gin.Context) {
	vars := gonja.Context{
		"title": "Setup intial user",
		"email": "",
		"name":  "",
		"team":  ""}
	...
	c.RenderAuth("users/setup", vars, gctx)
}

func (c *Controller) FinishInitialUserSetup(gctx *gin.Context) {
	email := gctx.PostForm("email")
	password := gctx.PostForm("password")
	...
	if len(errors) == 0 {
		...
		if user.ID == 0 {
			...
			vars := gonja.Context{
				"subject": "Welcome to gosass!",
				"name":    user.Name,
				"email":   user.Email,
			}

			utils.SendEmail("Welcome to gosass!", "", []string{user.Email}, vars, "welcome")
